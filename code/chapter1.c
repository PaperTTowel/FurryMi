...
void chapter1(HINSTANCE hInstance,char name[10]){
    static char key;
    printf("\t챕터1\n================================================================================\n\n\n");
    Sleep(1500);
...
return1:
    ...
    key = getch();
    gotoxy(1, 4), printf("그와중에 어떤 새끼가 열어놓았던 창문을 닫아 두어다. 미친 것이 분명하다. 나는 바로 달려가 창문을 열었다,\n");
    key = getch();
    gotoxy(1, 4), printf("\"와... 좀 살것 같다...\"                                                                                             \n");
    key = getch();
    ...
    key = getch();
    gotoxy(1, 4), printf("학교에서 털 때문에 체력이 많이 줄은 나는 배가 많이 고픈 나는 무엇을 점심으로 먹을지 고민을 하면서 점심시간만을 기다리고 있었다.       \n");
    key = getch();
    ...
    key = getch();
    gotoxy(1, 4), printf("내 점심은 멜론 빵이다. 그리고 학교 자판기에서 뽑아온 이온 음료다.          \n");
    key = getch();
    ...
    key = getch();
    gotoxy(1, 4), printf("옥상 밴치에 쭈꾸려 앉아서 혼자 빵을 먹고 있는 찐다.                    \n");
    key = getch();
    ...
    gotoxy(1, 4), printf("누군가가 뒤를 돌아보려는 내 머리를 강타했다.                 \n");
    key = getch();
    ...
    key = getch();
    gotoxy(1, 4), printf("그곳에는 히비키와 카스미가 서 있었다.                    \n");
    ...
    gotoxy(1, 4), printf("카스미의 장난섞인 말이 존나 쌔게 내 가슴에 날라와 박혔다.              \n");
    key = getch();
    ...
    key = getch();
    gotoxy(1, 4), printf("히비키의 얼굴에는 당황하는 표정이 고스란히 나와있었다.             \n");
    key = getch();
    ...
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9);
    gotoxy(1, 4), printf("\"가...같이... 점심 먹으려고..요...\"                    \n");
    key = getch();
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
...
return2:
    gotoxy(1, 4), printf("히비키와 카스미가 내가 앉아 있는 벤치에 앉았다.                                                           \n");
    key = getch();
    gotoxy(1, 4), printf("히비키와 카스미의 손에는 도시락통이 들려있었다. 각자의 개성이 뿜어져 나오는 도시락통이다.     \n");
    key = getch();

    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 13);
    gotoxy(1, 4), printf("\"응? %s는 멜론빵이랑 에너지 드링크가 점심인거야?\"                                           \n", name);
    key = getch();
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
    ...
    key = getch();
    gotoxy(1, 4),printf("다만 받은 반찬에서 조금의 털이 나왔다.                                \n");
    key = getch();
    gotoxy(1, 4), printf("음~ 역시 수인이 만든 음식이야.             \n");
    key = getch();
    gotoxy(1, 4), printf("털이 조금 묻은 점심을 다 먹은 우리는 오후 수업을 위해 각자의 반으로 돌아갔다.                \n");
    key = getch();
    gotoxy(1, 4), printf("아무리 봐도 학교에 반마다 고성능 공기청정기가 필요할 것 같다.         \n");
    key = getch();
